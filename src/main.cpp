...
std::string keys =
    ...
    "{ @alias      | | An alias name of model to extract preprocessing "
    "parameters from models.yml file. }"
    "{ zoo         | models.yml | An optional path to file with preprocessing "
    "parameters }"
    "{ device      |  0 | camera device number. }"
    "{ input i     | | Path to input image or video file. Skip this argument "
    "to capture frames from a camera. }"
    "{ framework f | | Optional name of an origin framework of the model. "
    "Detect it automatically if it does not set. }"
    "{ classes     | | Optional path to a text file with names of classes to "
...
int main(int argc, char **argv)
{
  ...
  parser.about("Use this script to run object detection deep learning networks "
               "using OpenCV.");
  ...
  createTrackbar("Confidence threshold, %", kWinName, &initialConf, 99,
                 callback);
  ...
  const string source   = argv[8];                  // the source file name
  string::size_type pAt = source.find_last_of('.'); // Find extension point
  string::size_type sAt = source.find_last_of('/'); // Find extension slash
...
#else  // USE_THREADS
  if (asyncNumReq)
    CV_Error(Error::StsNotImplemented, "Asynchronous forward is supported only "
                 ...
                 int backend)
{
  ...
  if (outLayers.size() > 1 ||
      (outLayerType == "Region" && backend != DNN_BACKEND_OPENCV)) {
    ...
    for (std::map<int, std::vector<size_t>>::iterator it =
      ...
      NMSBoxes(localBoxes, localConfidences, confThreshold, nmsThreshold,
               nmsIndices);
              ...
              Mat &frame)
{
  ...
  rectangle(frame, Point(left, top - labelSize.height),
            Point(left + labelSize.width, top + baseLine), Scalar::all(255),
            FILLED);
  putText(frame, label, Point(left, top), FONT_HERSHEY_SIMPLEX, 0.5, Scalar());
}

void callback(int pos, void *) { confThreshold = pos * 0.01f; }
