...
def overlay(title, objs, get_color, labels, inference_time, inference_rate, layout):
    ...
    for obj in objs:
        ...
        bbox = obj.bbox.scale(1.0 / inference_width, 1.0 /
                              inference_height).scale(*layout.size)
...
def render_gen(args):
    ...
    filtered_labels = set(l.strip()
                          for l in args.filter.split(',')) if args.filter else None
    ...
    while True:
        ...
        if draw_overlay:
            ...
            output = overlay(title, objs, get_color, labels,
                             inference_time, inference_rate, layout)
        else:
            output = None
